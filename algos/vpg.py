...
from algos.agent import Actor
from misc import HyperParams as hp
from misc import VPGMemory


def VPG(bullet_env):

    # Set up environment
    env = gym.make(bullet_env)
    n_inputs = env.observation_space.shape[0]
    n_outputs = env.action_space.shape[0]
    epc_steps = int(hp.steps_per_epoch / num_procs())

    def get_actor_loss(dat):
        state, action, returns, _ = dat['states'], dat['actions'], \
            dat['returns'], dat['logp']

        action, logp = agent(state, action)
        actor_loss = -(logp * returns).mean()
    ...
    logger = EpochLogger(output_fname='vpg_data.txt')
    torch.manual_seed(500)
    np.random.seed(500)
    memory = VPGMemory(n_inputs, n_outputs, epc_steps)

    # Create policy network (only one - actor)
    agent = Actor(n_inputs, n_outputs)
    sync_params(agent)
    ...
    for epoch in range(hp.epochs):
        for t in range(epc_steps):
            action, logp = agent.step(torch.as_tensor(state, dtype=torch.float32))

            next_state, reward, done, _ = env.step(action)
            ep_returns += reward
            n_steps += 1

            memory.save(state, action, reward, logp)
